...
var sessionDefaults = {viewCameraProps: {viewAngle: 39, aspectRatio: 1.777, nearCutOff: 0.1,
                                         ...
                                         camUp: {x: 0, y: 0, z: 1}, fixedView: false,
                                         lookAt: {x: 0, y: 0, z: 0}
                                        },
                       selectData: {isDragging: false, isScaling: false, isRotating : false, direction: '', delta: 0},
                       objSelect:  {objName: '', isSelected: false, isEdit :false, editMode: '', wireframeMode : false },
                       vefSelect: {objName: '', isSelected: false },
                       ...
                       selObjLoc: {x: 0, y: 0}, selObjPos: {x: 0, y: 0, z: 0},
                       selObjScale: {x: 0, y: 0, z: 0}, selObjRot: {x: 0, y: 0, z: 0}
                      };
...
function initSceneData()
{
  ...
  sceneData.viewPort.style.cursor = 'crosshair';
  
  /* Get the dimensions of the view port */
  var width = $('#viewPort').width();
  var height = $('#viewPort').height();

  /* Set viewport size and background */
...
function render(normalizeReq)
{
  ...
  DebugInfo("render: Camera.rot.xyz (" + cam.rotation.x/PI + ", " + cam.rotation.y/PI + ", " + cam.rotation.z/PI + ")");
  
  /* Render */
  sceneData.viewRenderer.render(sceneData.viewPortScene, cam);
  
  /* If normalize is requested */
  if(normalizeReq == true)
  {
