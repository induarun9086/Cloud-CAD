    ...
    socket.on('export', function(msg) {
        ...
        cursor.count(function (err, count) {
            if (err != null || count == null || count == 0) {
                ...
                dataBase.collection('accounts').replaceOne({"sceneName": exportData.sceneName}, exportData, function (err, db) {
                    if (err) {
                        throw err;
                    }
        ...
        cursor.count(function (err, count) {
            if (err == null && count > 0) {
                cursor.next(function (err, data) {
                    ...
                    console.log('Retrieved the requested scene ' + data.sceneName);
